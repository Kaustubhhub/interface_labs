...
const page = () => {
  ...
  const countLastMonthOrder = (data: any) => {
    ...
    const formattedData = lastMonthOrders.map((row: any) => {
      return [
        row.id,               // Order ID
        row.total ?? "",                   // Net Amount (assuming it's empty as no data is provided)
        row.invoiceAmount,    // Invoice Amount
        new Date(row.orderDate).toLocaleDateString(), // Order Date, formatted
        row.description,      // Description
        ''                    // Payment Details (assuming it's empty as no data is provided)
      ];
    });
